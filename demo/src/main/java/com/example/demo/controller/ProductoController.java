...
@RequestMapping("/productos") //Cuando se baya a hacer una peticion de este controlador las url deben tener un /producto
public class ProductoController { 
    ...
    @Autowired
    private CategoriaService categoriaService; //Utilizamos la variable para llamar a todos los metodos de CategoriaService

    // Mostrar la ista con los productos
    @GetMapping
    public String listar(Model model) {
        model.addAttribute("productos", productoService.obtenerTodos()); //Llama al servicio de productoService, a la funcion obtener todos
    ...
    public String mostrarFormularioNuevo(Model model) { //mostramos el formulario para el nuevo producto
        ...
        model.addAttribute("categorias", categoriaService.findAll()); //Cuando creemos un nuevo plato lo tenemos que clasificar en una nueva categoria, llamamos a categoriaService para que nos muestre todas
    ...
    public String guardar(@ModelAttribute Producto producto) { //Convierte los datos en un objeto producto
        productoService.guardar(producto); //Luego con ayuda de productoService guardamos ese objeto con la nueva informacion del formulario
